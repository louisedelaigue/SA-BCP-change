...
for var in ds.keys():
    ...
    ds_mod[var] = (("gamma", "datenum", "cruise_mod"), var_data)
    
# Upgrade array slices to xarray ds
ds_mod = xr.Dataset(ds_mod, coords={"gamma":ds.gamma.values, "datenum":ds.datenum.values, "cruise_mod":ds.cruise.values})
...
vars_errors = xr.Dataset(vars_errors, coords={"gamma":ds.gamma.values})
...
vars_errors.to_netcdf("data/analysis/A02_calc_change_derivatives.nc")
...
#     # Improve figure
#     ax.set_xlim(change.gamma.min(), change.gamma.max())
#     # ax.set_xlim(26, 28.1)
