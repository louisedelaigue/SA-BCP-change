...
derivatives = xr.load_dataset("data/analysis/GTC02_calc_change_derivatives.nc")
bootstrapping = xr.load_dataset("data/analysis/GTC03_bootstrapping.nc")
...
std_errors = xr.Dataset(coords={"depth": ds["depth"]})
slopes = xr.Dataset(coords={"depth": ds["depth"]})

for varname, var in ds[[k for k in ds.keys() if k != "cruise" and "_bias" not in k]].data_vars.items():
    # Create an empty array to store the stderr values for each depth
    stderr = np.empty(ds.dims["depth"])
    slope = np.empty(ds.dims["depth"])

    for i, depth in enumerate(ds["depth"]):
        # Compute the linear regression for the current depth and variable
        x = ds["datenum"]
        y = var.sel(depth=depth)
        slope = stats.linregress(x, y)[0]
        stderr[i] = stats.linregress(x, y)[4]

    # Add the computed stderr as a new variable to the results dataset
    std_errors[varname+"_error"] = xr.DataArray(stderr, coords=[ds["depth"]], dims=["depth"])
    slopes[varname+"_change"] = xr.DataArray(slope, coords=[ds["depth"]], dims=["depth"])
...
change_error.to_netcdf("data/analysis/GTC04_combine_errors.nc")
...
for v in change_error.keys():
    ...
    if "_error" in v:    
        ...
        ax.set_xlim(change_error.depth.min(), change_error.depth.max())
